...
let CourseVideoService = class CourseVideoService {
  ...
  async save(data) {
    ...
    const existingPart = await this.videoModel.findOne({
      part: data.part,
      lesson: data.lesson,
    });
    if (existingPart) {
    ...
    try {
      const resultVideo = await this.cloudinaryService.uploadFile(
        data.file,
        cloudinary_constants_1.FILE_COURSE_VIDEO,
        fileVideo.fieldname,
  ...
  async updateById(id, data) {
    try {
      ...
      if (data.part != null) {
        const existingSelection = await this.videoModel.findOne({
          part: data.part,
        });
      ...
      const valueFind = await this.videoModel
